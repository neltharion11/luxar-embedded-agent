...
class TaskRouterTests(unittest.TestCase):
    ...
    def test_fix_file_issue_routes_to_review_or_fix(self) -> None:
        ...
        self.assertEqual("review_or_fix", plan.intent.intent_type)
...
class RunTaskTests(unittest.TestCase):
    def test_run_task_greeting_returns_human_friendly_message(self) -> None:
        ...
        self.assertEqual("explain", result["intent"])
    ...
    def test_run_task_build_request_uses_build_project_not_debug_loop(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("luxar.tools.run_task.ProjectManager") as pm_cls, \
                 patch("luxar.tools.run_task.run_build_project", return_value=build_result) as build_mock, \
                 patch("luxar.tools.run_task.run_debug_workflow") as debug_mock:
                pm_cls.return_value.load_project.return_value = ProjectConfig(
                    name="DirectF1C",
                    path=str(Path(tmpdir) / "DirectF1C"),
                    project_mode="firmware",
    ...
    def test_run_task_generate_driver_uses_driver_pipeline_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("luxar.tools.run_task.run_generate_driver_loop", return_value=pipeline):
                result = run_task(
                    config=config,
                    project_root=tmpdir,
                    workspace_root=tmpdir,
        ...
        self.assertTrue(result["success"])
        self.assertEqual("generate_driver", result["intent"])
        self.assertIn("artifacts", result)
    ...
    def test_run_task_auto_fixes_known_review_issues(self) -> None:
        ...
        try:
            ...
            with patch("luxar.tools.run_task.ProjectManager") as pm_cls, \
                 patch("luxar.tools.run_task.run_review_project", side_effect=[first_report, second_report]), \
                 patch("luxar.tools.run_task.run_fix_code", return_value=mock.Mock(success=True, applied=True)) as fix_mock:
                pm_cls.return_value.load_project.return_value = ProjectConfig(
                    name="DirectF1C",
                    path=project_path,
                    project_mode="firmware",
    ...
    def test_run_task_respects_empty_auto_fix_whitelist(self) -> None:
        ...
        try:
            ...
            with patch("luxar.tools.run_task.ProjectManager") as pm_cls, \
                 patch("luxar.tools.run_task.run_review_project", return_value=report), \
                 patch("luxar.tools.run_task.run_fix_code") as fix_mock:
                pm_cls.return_value.load_project.return_value = ProjectConfig(
                    name="DirectF1C",
                    path=project_path,
                    project_mode="firmware",
