...
from luxar.agent.workers.planner import PlannerWorker
...
def run_forge_project_stream(
    *,
    ...
    planner = PlannerWorker(config)
    
    plan_skill_path = Path(project_root).resolve() / "workspace" / "skills" / "workflows" / "plan_project" / "SKILL.md"
    plan_skill = plan_skill_path.read_text(encoding="utf-8") if plan_skill_path.exists() else "Plan project conservatively."

    project_plan = planner.build_plan(
        project=project,
        requirement=requirement,
        skill_instructions=plan_skill,
    ...
    project_plan = project_plan.model_copy(update={"needed_drivers": planned_driver_requirements})
    ...
    advisor = AssetReuseAdvisor(
        project_root=project_root,
        driver_library_root=driver_library_root,
        skill_library_root=str(Path(project_root).resolve() / config.agent.skills_root),
        legacy_skill_library_root=str(Path(project_root).resolve() / config.agent.skill_library),
    )
    ...
    generator = GeneratorWorker(config)
    
    app_skill_path = Path(project_root).resolve() / "workspace" / "skills" / "workflows" / "generate_app" / "SKILL.md"
    app_skill = app_skill_path.read_text(encoding="utf-8") if app_skill_path.exists() else "Generate app_main.h and app_main.c"
    ...
    internal = "\n".join(f"- {item.interface} {item.instance} mode={item.mode} pins={item.pins}" for item in project_plan.internal_peripherals) or "- none"
    board = "\n".join(f"- {item.instance} mode={item.mode} pins={item.pins}" for item in project_plan.board_features) or "- none"
    bindings = "\n".join(f"- {item.device}: {item.driver} over {item.transport} via {item.peripheral}" for item in project_plan.transport_bindings) or "- none"

    context_data = {
        "requirement_summary": project_plan.requirement_summary,
        "installed_drivers": ", ".join(installed_driver_names) or "none",
        "features": features,
        "app_behavior_summary": project_plan.app_behavior_summary,
        "document_context_summary": project_plan.document_context_summary or "None",
        "peripheral_hints": peripheral_hints,
        "internal_peripherals": internal,
        "board_features": board,
        "transport_bindings": bindings,
        "pending_configuration_actions": config_actions,
        "risk_notes": risks,
        "mcu": project.mcu,
    }

    gen_out = generator.generate_files(
        context_data=context_data,
        skill_instructions=app_skill,
        output_dir=str(Path(project.path) / "App" / "Inc"), 
        stem="app_main",
    )
    
    # The GeneratorWorker outputs files to the same directory.
    # We need to move the source file to App/Src
    header_path = ""
    source_path = ""
    if gen_out.get("success"):
        import shutil
        header_path = gen_out["header_path"]
        orig_source_path = gen_out["source_path"]
        
        src_dir = Path(project.path) / "App" / "Src"
        src_dir.mkdir(parents=True, exist_ok=True)
        new_source_path = src_dir / "app_main.c"
        if Path(orig_source_path).exists() and Path(orig_source_path).resolve() != new_source_path.resolve():
            shutil.move(orig_source_path, new_source_path)
            source_path = str(new_source_path)
        else:
            source_path = orig_source_path
    
    from luxar.models.schemas import AppGenerationResult
    app_result = AppGenerationResult(
