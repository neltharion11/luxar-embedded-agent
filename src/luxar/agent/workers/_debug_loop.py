        ...
        for file_path, report in reports.items():
            try:
                report_json = json.dumps({
                    ...
                    "issues": [issue.model_dump(mode="json") if hasattr(issue, "model_dump") else dict(issue) for issue in report.issues],
                    "raw_logs": {},
                }, ensure_ascii=False)
                
                fix_result = worker.repair_file(
                    file_path=file_path,
                    context_report=report_json,
                    skill_instructions="Fix the compilation errors in the report. Prioritize fatal errors. Provide the complete source file.",
                    apply_changes=True,
                )
            except Exception as exc:
                actions.append(f"Build-fix attempt for {Path(file_path).name} was skipped: {exc}")
                continue
            if fix_result.get("success") and fix_result.get("applied"):
